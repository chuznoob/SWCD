...
{
  $id_are=$_POST['id']; 
  $nomOld_area=$_POST['nom']; 
  
$classAreas = new Area();
$dataArea = $classAreas -> showData($id_are);

while($area=$dataArea->fetchObject())
      {
          $id_area=$area->id_area;
          $nom_area=$area->nom_area;
          $uc_area=$area->uc_area;
          $fc_area=$area->fc_area;
          $um_area=$area->um_area;
          $fm_area=$area->fm_area;
      }
}else{
          $id_area=NULL;
          $nom_area=NULL;
...
<head>
	...
	<!-- Titulo Pestaña=================== -->
	<title>S.W.C.D/Áreas/Formulario</title>
			...
			<div class="content-wrap">
				...
				<div class="container clearfix">
					...
					<div class="col_two_third nobottommargin">
						...
						<h3><I class="icon-line-clipboard"></I> Formulario de áreas</h3>
						
						<?php if($id_area!=NULL){ ?> 
							    <p><i class="icon-edit cinfo"></i> Ingresa el nuevo nombre para el área</p>
							    <div class="alert sm alert-warning">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-warning-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>último editor</strong> de la área
						        </div>
							    <?php }else{ ?>
							    <p><i class="icon-edit cinfo"></i> Ingresa el nombre de la nueva área</p>
							    <div class="alert sm alert-info">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-info-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>creador</strong> de este nueva área
						        </div>
							    <?php } ?>
						<form id="register-form" name="addData" class="nobottommargin" 
                        action="../lib/ops_area/UpAddArea.php" method="post">

							<div class="col_full">
								<label for="nom_area">Nombre del área:</label>
								<input type="text" class="form-control" name="nom_area" maxlength="99" 
								<?php if($id_area!=NULL){ ?> 
							    value="<?php echo $nom_area; ?>"
							    <?php }else{ ?>
							value="" placeholder="Ingresa el nombre del área"
							<?php } ?> required/>
							</div>
							...
							<?php if($id_area!=NULL){ ?> 
							<input type="hidden" name="nomOld_area" 
							value="<?php echo $nomOld_area; ?>"/>
							<?php }?>
							
							<input type="hidden" name="uc_area" 
							<?php if($id_area!=NULL){ ?> 
							value="<?php echo $uc_area; ?>"
							<?php }else{ ?>
							value="<?php echo $_SESSION['id_usuario']; ?>"
							<?php } ?>/>
							
							<input type="hidden" name="fc_area" 
							<?php if($id_area!=NULL){ ?> 
							value="<?php echo $fc_area; ?>"
							<?php }else{ ?>
							id="horafechaval" 
							<?php } ?>/>
							
							<input type="hidden" name="um_area" 
							<?php if($id_area!=NULL){ ?> 
							value="<?php echo $_SESSION['id_usuario']; ?>"
							<?php }else{ ?>
							value="NULL"
							...
							<input type="hidden" name="fm_area" 
							<?php if($id_area!=NULL){ ?> 
							id="horafechaval" 
							<?php }else{ ?>
							value="NULL"
							...
							<div class="col_full nobottommargin">
								<button type="submit" class="button button-blue button-rounded button-reveal tright">
								<i class="icon-checkmark"></i><span>Enviar</span></button>
							</div>
