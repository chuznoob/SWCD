		...
		<section id="content">
			...
			<div class="content-wrap-short">
				...
				<div class="container clearfix">
                   ...
                   <a href="form-secciones.php" class="button button-rounded button-reveal button-teal button-border tright"><i class="icon-plus-sign2"></i><span>Agregar Sección</span></a></div>
                    ...
                    <div class="col_full">
						<div class="panel panel-default">
                            ...
                            <?php
                            if($dataSeccion != NULL){
                            while($data=$dataSeccion->fetchObject()){
                                
                                if($data->uc_seccion==NULL){
                                    $uc_seccion="Sistema";
                                }else{
                                $idc=$data->uc_seccion;
                                ...
                                if($data->fc_seccion==NULL){
                                    $fc_seccion="Creado con el sistema";
                                }else{
                                    $fecha = DateTime::createFromFormat('Y-m-d H:i:s', $data->fc_seccion);
                                    $ts= $fecha->getTimestamp();
                                    $fc_seccion=strftime("%d %B del %Y <br> %I:%M:%S %p",$ts);
                                }
                               
                                if($data->um_seccion==NULL){
                                    $um_seccion="Sin Modificaciones";
                                }else{
                                $idm=$data->um_seccion;
                                $classUsuario2 = new Usuario();
                                $dataUsuario2 = $classUsuario2 -> showData($idm);
                                $dataU2=$dataUsuario2->fetchObject();
                                $um_seccion=$dataU2->nom_usuario;
                                }
                                
                                 if($data->fm_seccion==NULL){
                                    $fm_seccion="Sin Modificaciones";
                                }else{
                                    $fecha = DateTime::createFromFormat('Y-m-d H:i:s', $data->fm_seccion);
                                    $ts= $fecha->getTimestamp();
                                    $fm_seccion=strftime("%d %B del %Y <br> %I:%M:%S %p",$ts);
                                }
                                
                                 $pathSimpl_documento=str_replace('mods/', '', $data->path_seccion);
                                
                            echo "<tr class='odd gradeX'>";
                            echo "<td>" . $data->nom_seccion . "</td>";
                            echo "<td>" . $pathSimpl_documento . "</td>";
                            echo "<td><Strong>Creador:</Strong><br>" . $uc_seccion . "<br>
                                      <Strong>Fecha:</Strong><br>" . $fc_seccion . "</td>";
                            echo "<td><Strong>Modificador:</Strong><br>" .$um_seccion . "<br>
                                      <Strong>Fecha:</Strong><br>" . $fm_seccion . "</td>";
                            if($data->id_seccion==1){
                            echo "<td class='center'>
                            <button type='submit' class=' button button-rounded button-reveal button-black tright'><i class='icon-forbidden'></i><span>No disponible</span></button>   
                            </td>";
                            }else{
                            echo "<td class='center'>
                            <form name='F-update' action='form-secciones.php' method='post'>
                            <input type='hidden' name='id' value='".$data->id_seccion."'>
                            <input type='hidden' name='nom' value='".$data->nom_seccion."'>
                            ...
                            echo "<td class='center'>
                            <form name='F-update' action='../lib/ops_seccion/deleteSeccion.php' method='post'>
                            <input type='hidden' name='id' value='".$data->id_seccion."'>
                            <input type='hidden' name='nom' value='".$data->nom_seccion."'>
    ...
    <script>
        $(document).ready(function() {
            $('#dataTables-example2').dataTable( {
                ...
                "oLanguage": {
                    "sLengthMenu": " _MENU_ Secciones por p&aacute;gina"
                }
            } );
