...
<head>
	...
	<!-- Titulo Pestaña=================== -->
	<?php if($bread=="explorador"){ ?>
    <title>S.W.C.D/Menú de documentos/Agregar-Explorar/Formulario</title>
    <?php }else{ if($bread=="version"){ ?>
    <title>S.W.C.D/Menú de documentos/Nueva-versión/Formulario</title>
    <?php }else{ if($bread=="editor"){ ?>
    <title>S.W.C.D/Menú de documentos/Editar-Eliminar/Formulario</title>
    <?php } } } ?>
...
<body class="stretched side-panel-left" data-loader="4" data-loader-color="#999">
		...
		<section id="page-title" class="page-title-mini">
			...
			<div class="container clearfix">
				...
				<ol class="breadcrumb">
					...
					<?php if($bread=="explorador"){ ?>
					<li><a href="explorador-documentos.php">Agregar-Explorar</a></li>
					<?php }else{ if($bread=="version"){ ?>
                    <li><a href="version-documentos.php">Nueva-Versión</a></li>
                    <?php }else{ if($bread=="editor"){ ?>
                    <li><a href="editor-documentos.php">Editar-Eliminar</a></li>
                    <?php } } } ?>
					<li>Formulario</li>
			...
			<div class="content-wrap">
				...
				<div class="container clearfix">
					...
					<div class="col_two_third nobottommargin">
						...
						<h3><I class="icon-line-clipboard"></I> Formulario de documentos</h3>
						
						<?php if($id_documento!=NULL){ ?> 
							    <p><i class="icon-edit cinfo"></i>Ingresa los datos necesarios para la edición del documento</p>
							    <div class="alert sm alert-warning">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-warning-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>último editor</strong> del documento
						        </div>
							    <?php }else{ ?>
							    <p><i class="icon-edit cinfo"></i> Ingresa los datos necesarios para el registro del nuevo documento</p>
							    <div class="alert sm alert-info">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-info-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>creador</strong> de este nuevo documento
						        </div>
							    <?php } ?>
						<form id="register-form" name="addData" class="nobottommargin" 
                        action="../lib/ops_documento/UpAddDocumento.php" method="post" 
                        enctype="multipart/form-data">

							<div class="col_half">
								<label for="nom_documento">Nombre del documento:</label>
								<input type="text" class="form-control" name="nom_documento" id="nom_documento" maxlength="199" 
								<?php if($id_documento!=NULL){ ?> 
							    value="<?php echo $nom_documento; ?>" <?php if($bread=="version"){ echo "readonly"; } ?>
							    <?php }else{ ?>
							value="" placeholder="Ingresa el nombre del documento"
							<?php } ?> required/>
							</div>
							
							<div class="col_half col_last">
								<label for="ver_documento">Version del documento:</label>
								<input type="number" class="form-control" name="ver_documento" 
								<?php if($id_documento!=NULL){ 
                                      if($bread=="editor"){ ?>
                                           min="<?php echo $ver_documento; ?>" max="<?php echo $ver_documento; ?>" 
							    value="<?php echo $ver_documento; ?>" readonly
                                     <?php }else{ $newver_documento=$ver_documento+1 ?> 
							    min="<?php echo $newver_documento; ?>" max="<?php echo $newver_documento; ?>" 
							    value="<?php echo $newver_documento; ?>" readonly
                                     <?php } }else{ ?>
							 min="1" max="1" value="1"
							<?php } ?> required/>
							</div>
							...
							<div class="col_full">
								<label for="mod_documento">Escribe las caracteristicas del documento:</label>
								<?php if($id_documento!=NULL){ ?> 
                            <textarea type="text" class="form-control" name="mod_documento" style="resize:vertical" maxlength="50000" minlength="10" required><?php echo $mod_documento; ?></textarea>
							    <?php }else{ ?>
							<textarea type="text" class="form-control" name="mod_documento" style="resize:vertical" maxlength="50000" minlength="10" required></textarea>
							<?php } ?>
							</div>
							
							<div class="col_half">
								<label for="sec_documento">Sección del documento:</label>
								<select name="sec_documento" class="form-control" <?php if($id_documento == NULL){echo "readonly";} ?>>
				...
				<?php if($sec->id_seccion==$sec_documento){ echo "selected"; } ?> ><?php echo $sec->nom_seccion; ?></option>
                                    <?php } }else{ if($sec->nom_seccion == $sec_doc){  ?>
                                    <option value="<?php echo $sec->id_seccion; ?>"
                <?php if($sec->nom_seccion==$sec_doc){ echo "selected";} ?> ><?php echo $sec->nom_seccion; ?></option>
                                    <?php } } } } ?>
							...
							<div class="col_half col_last">
								<label for="path_documento">Destino del documento:</label>
								<input type="text" class="form-control" name="path_simple" 
								<?php if($id_documento!=NULL){ ?> 
				            ...
				            <?php if($bread=="editor"){ }else{ ?>
							<div class="col_full">
								<label for="document">Seleccionar documento:</label>
								<input type="file" class="form-control" name="document"  accept="audio/*|video/*|image/*"/>
							</div>
							<?php } ?>
							...
							<?php if($id_documento!=NULL){ ?> 
							<input type="hidden" name="nomOld_doc" 
							value="<?php echo $nomOld_doc; ?>"/>
							<?php }?>
							
							<input type="hidden" name="path_documento" 
                            <?php if($id_documento!=NULL){ ?> 
				            value="<?php echo $path_documento; ?>"
				            <?php }else{ ?>
				            value="<?php echo $pathOld_doc; } ?>">
							   
							 <input type="hidden" name="Oldpath_documento" 
                             <?php if($id_documento!=NULL){ ?> 
				             value="<?php echo $path_documento; ?>"
				             <?php }else{ ?>
				             value="NULL" <?php } ?>>
							   
							   <input type="hidden" name="Oldext_documento" 
                               <?php if($id_documento!=NULL){ ?> 
							   value="<?php echo $ext_documento; ?>"
							    <?php }else{ ?>
							   value="NULL" <?php } ?>>
							
							<input type="hidden" name="uc_documento" 
							<?php if($id_documento!=NULL){ ?> 
							...
							<input type="hidden" name="fc_documento" 
							<?php if($id_documento!=NULL){ ?> 
							...
							<input type="hidden" name="um_documento" 
							<?php if($id_documento!=NULL){ ?> 
							value="<?php echo $_SESSION['id_usuario']; ?>"
							<?php }else{ ?>
							value="NULL"
							...
							<input type="hidden" name="fm_documento" 
							<?php if($id_documento!=NULL){ ?> 
							id="horafechaval" 
							<?php }else{ ?>
							value="NULL"
    ...
    <script>
    $('#nom_documento').bind('keypress', function (event) {
