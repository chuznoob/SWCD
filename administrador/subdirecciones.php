...
<head>
	...
	<!-- Titulo Pestaña=================== -->
	<title>S.W.C.D/Subdirecciones / Direcciones</title>
...
<body class="stretched side-panel-left" data-loader="4" data-loader-color="#999">
	...
	<div id="wrapper" class="clearfix">
		...
		<section id="page-title" class="page-title-mini">
			...
			<div class="container clearfix">
				<h1><i class="icon-map"></i> Subdirecciones / Direcciones</h1>
				<!-- Menu=================== -->
				<ol class="breadcrumb">
					<li><a href="index.php">Administrador</a></li>
					<li>Subdirecciones / Direcciones</li>
				</ol>
                <!-- /Menu================== -->
			</div>
		...
		<section id="content">
			...
			<div class="content-wrap-short">
				...
				<div class="container clearfix">
                   ...
                   <div class="col_two_third"><h4>
                   <i class="icon-grid"></i> Tabla de elementos</h4></div>
                   <div class="col_one_third col_last center">
                   <a href="form-subdirecciones.php" class="button button-rounded button-reveal button-teal button-border tright"><i class="icon-plus-sign2"></i><span>Agregar Subdireccion/Direccion</span></a></div>
                    ...
                    <div class="col_full">
						<div class="panel panel-default">
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="dataTables-example2">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>Procesos ligados a <br>Subdirecciones / Direcciones </th>
                                                    <th>Editar</th>
                                                    <th>Borrar</th>
                                                </tr>
                                            </thead>
                            ...
                            while($data=$dataSubdir->fetchObject()){
                                ...
                                if($data->uc_subdireccion==NULL){
                                    $uc_subdireccion="Sistema";
                                }else{
                                $idc=$data->uc_subdireccion;
                                $classUsuario = new Usuario();
                                $dataUsuario = $classUsuario -> showData($idc);
                                $dataU=$dataUsuario->fetchObject();
                                $uc_subdireccion=$dataU->nom_usuario;
                                }
                                
                                if($data->fc_subdireccion==NULL){
                                    $fc_subdireccion="Creado con el sistema";
                                }else{
                                    $fecha = DateTime::createFromFormat('Y-m-d H:i:s', $data->fc_subdireccion);
                                    $ts= $fecha->getTimestamp();
                                    $fc_subdireccion=strftime("%d %B del %Y <br> %I:%M:%S %p",$ts);
                                }
                               
                                if($data->um_subdireccion==NULL){
                                    $um_subdireccion="Sin Modificaciones";
                                }else{
                                $idm=$data->um_subdireccion;
                                $classUsuario2 = new Usuario();
                                $dataUsuario2 = $classUsuario2 -> showData($idm);
                                $dataU2=$dataUsuario2->fetchObject();
                                $um_subdireccion=$dataU2->nom_usuario;
                                }
                                
                                 if($data->fm_subdireccion==NULL){
                                    $fm_subdireccion="Sin Modificaciones";
                                }else{
                                    $fecha = DateTime::createFromFormat('Y-m-d H:i:s', $data->fm_subdireccion);
                                    $ts= $fecha->getTimestamp();
                                    $fm_subdireccion=strftime("%d %B del %Y <br> %I:%M:%S %p",$ts);
                                }
                                
                            echo "<tr class='odd gradeX'>";
                            echo "<td>" . $data->nom_subdireccion . "</td>";
                            echo "<td><Strong>Creador:</Strong><br>" . $uc_subdireccion . "<br>
                                      <Strong>Fecha:</Strong><br>" . $fc_subdireccion . "</td>";
                            echo "<td><Strong>Modificador:</Strong><br>" .$um_subdireccion . "<br>
                                      <Strong>Fecha:</Strong><br>" . $fm_subdireccion . "</td>";
                            echo "<td class='center'>
                            <form name='F-update' action='rel-subdirecciones.php' method='post'>
                            <input type='hidden' name='id' value='".$data->id_subdireccion."'>
                            <input type='hidden' name='nom' value='".$data->nom_subdireccion."'>
                            ...
                            echo "<td class='center'>
                            <form name='F-update' action='form-subdirecciones.php' method='post'>
                            <input type='hidden' name='id' value='".$data->id_subdireccion."'>
                            <input type='hidden' name='nom' value='".$data->nom_subdireccion."'>
                            ...
                            echo "<td class='center'>
                            <form name='F-update' action='../lib/ops_subdireccion/deleteSubdireccion.php' method='post'>
                            <input type='hidden' name='id' value='".$data->id_subdireccion."'>
                            <input type='hidden' name='nom' value='".$data->nom_subdireccion."'>
...
<?php unset($_SESSION['errorOp']); unset($_SESSION['detOp']); }else{ 
 ...
 <div id="mensaje" data-notify-position="top-right" 
     data-notify-type="error" 
     data-notify-msg="<i class='icon-remove'></i> La operación de <strong><?php echo $_SESSION['detOp']; ?></strong> no se completó, por favor verifica si la subdirección no está siendo usada por un usuario , ya que no es posible borrar una subdirección que está siendo usada"></div>
    ...
    <script>
        $(document).ready(function() {
            $('#dataTables-example2').dataTable( {
                ...
                "oLanguage": {
                    "sLengthMenu": " _MENU_ Subdirecciones por p&aacute;gina"
                }
            } );
