...
{
  $id_subdir=$_POST['id']; 
  $nomOld_subdir=$_POST['nom']; 
  
$classSubdirs = new Subdireccion();
$dataSubdir = $classSubdirs -> showData($id_subdir);

while($subdir=$dataSubdir->fetchObject())
      {
          $id_subdireccion=$subdir->id_subdireccion;
          $nom_subdireccion=$subdir->nom_subdireccion;
          $uc_subdireccion=$subdir->uc_subdireccion;
          $fc_subdireccion=$subdir->fc_subdireccion;
          $um_subdireccion=$subdir->um_subdireccion;
          $fm_subdireccion=$subdir->fm_subdireccion;
      }
}else{
          $id_subdireccion=NULL;
          $nom_subdireccion=NULL;
          $uc_subdireccion=NULL;
          $fc_subdireccion=NULL;
          $um_subdireccion=NULL;
          $fm_subdireccion=NULL; 
}
...
<head>
	...
	<!-- Titulo Pestaña=================== -->
	<title>S.W.C.D/Subdirecciones / Direcciones/Formulario</title>
			...
			<div class="content-wrap">
				...
				<div class="container clearfix">
					...
					<div class="col_two_third nobottommargin">
						...
						<h3><I class="icon-line-clipboard"></I> Formulario de subdirección</h3>
						
						<?php if($id_subdireccion!=NULL){ ?> 
							    <p><i class="icon-edit cinfo"></i> Ingresa el nuevo nombre para la subdirección</p>
							    <div class="alert sm alert-warning">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-warning-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>último editor</strong> de la subdirección
						        </div>
							    <?php }else{ ?>
							    <p><i class="icon-edit cinfo"></i> Ingresa el nombre de la nueva subdirección</p>
							    <div class="alert sm alert-info">
						          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
						          <i class="icon-info-sign"></i> El sistema tomara tus datos para reconocerte como el <strong>creador</strong> de este nueva subdirección
						        </div>
							    <?php } ?>
						<form id="register-form" name="addData" class="nobottommargin" 
                        action="../lib/ops_subdireccion/UpAddSubdireccion.php" method="post">

							<div class="col_full">
								<label for="nom_subdireccion">Nombre de la subdirección:</label>
								<input type="text" class="form-control" name="nom_subdireccion" maxlength="99" 
								<?php if($id_subdireccion!=NULL){ ?> 
							    value="<?php echo $nom_subdireccion; ?>"
							    <?php }else{ ?>
							value="" placeholder="Ingresa el nombre de la subdirección"
							<?php } ?> required/>
							</div>
							
							
							<?php if($id_subdireccion!=NULL){ ?> 
							<input type="hidden" name="id_subdireccion" 
							value="<?php echo $id_subdireccion; ?>"/>
							<?php }?>
							
							<?php if($id_subdireccion!=NULL){ ?> 
							<input type="hidden" name="nomOld_subdir" 
							value="<?php echo $nomOld_subdir; ?>"/>
							<?php }?>
							
							<input type="hidden" name="uc_subdireccion" 
							<?php if($id_subdireccion!=NULL){ ?> 
							value="<?php echo $uc_subdireccion; ?>"
							<?php }else{ ?>
							value="<?php echo $_SESSION['id_usuario']; ?>"
							<?php } ?>/>
							
							<input type="hidden" name="fc_subdireccion" 
							<?php if($id_subdireccion!=NULL){ ?> 
							value="<?php echo $fc_subdireccion; ?>"
							<?php }else{ ?>
							id="horafechaval"
							<?php } ?>/>
							
							<input type="hidden" name="um_subdireccion" 
							<?php if($id_subdireccion!=NULL){ ?> 
							value="<?php echo $_SESSION['id_usuario']; ?>"
							<?php }else{ ?>
							value="NULL"
							...
							<input type="hidden" name="fm_subdireccion" 
							<?php if($id_subdireccion!=NULL){ ?> 
							id="horafechaval"
							<?php }else{ ?>
							value="NULL"
